...
export default function User() {
  ...
  const UserTop = () => {
    ...
    // state: 닉네임 상태
    const [nickname, setNickname] = useState<string>('false');
    // state: 변경 닉네임 상태
    const [changeNickname, setChangeNickname] = useState<string>('false');
    // state: 프로필 이미지 상태
    ...
    // effect: useremail path variable 변경시 실행 할 함수
    useEffect(() => {
      if(!userEmail) return;
      getUserRequest(userEmail).then(getUserResponse);
          ...
          <div className='user-top-my-profile-image-box' onClick={onProfileBoxClickHandler}>
            ...
            <div className='user-top-my-profile-image-nothing-box'>
              <div className='icon-box-large'>
                <div className='icon image-box-while-icon'></div>
              </div>
            </div>
            }
