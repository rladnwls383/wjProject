...
import { deleteBoardReqeust, getBoardRequest, getCommentListRequest, getFavoriteListRequest, increaseViewCountRequest, postCommentRequest, putFavoriteRequest } from 'apis';
...
export default function BoardDetail() {
  ...
  const BoardDetailTop = () => {
    ...
    const getBoardResponse = (responseBody: GetBoardResponseDto | ResponseDto | null) => {
      if (!responseBody) return;
      const { code } = responseBody;
      if (code === 'NB') alert('존재하지 않는 계시물입니다.');
      if (code === 'DBE') alert('데이터베이스 오류입니다.');
      if (code !== 'SU') {
        navigate(MAIN_PATH());
        return;
    ...
    const onDeleteButtonClickHandler = () => {
      ...
      deleteBoardReqeust(boardNumber, cookies.accessToken).then(deleteBoardResponse);
    }

    // effect: 게시물 번호 path variable이 바뀔떄 마다 게시물 불러오기
    useEffect(() => {
      if (!boardNumber) {
        navigate(MAIN_PATH());
        return;
  ...
  // effect: 게시물 번호 path variable이 바뀔떄 마다 게시물 조회수 증가
  let effectFlag = true;
  useEffect(() => {
    if (!boardNumber) return;
    if (effectFlag) {
